...
<template>
    <div class="sub-tpl-manage-container">
        ...
        <el-row>
            ...
            <el-col :span="19" :offset="1" v-if="beforeStatus">
                <el-table height="calc(100vh - 450px)" ref="multipleTable" :data="tableData" @row-dblclick="handleOpenWordEdit" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" size="mini" border>
        ...
        <!-- 编辑打开word -->
        <subTplManageWordEdit :templateType='templateType' :isHandleDocCode='isHandleDocCode' :wordcode="wordcode" :dialogSortVisible="wordshow" v-if="wordshow" :currentTableData="currentTableData" @handleRefresh='handleRefresh' @closeSubTplWord='handlecloseSubTplWord'></subTplManageWordEdit>
        <!-- 新增、修改弹窗 -->
        <subTplManageDailog :currentTreeNode="currentTreeNode" :dialogFormVisible="show" :dailogType="dailogType" @closeSubManage="handleCloseDailog" :editRes="editRes" :tpck="tpck" v-if="show"></subTplManageDailog>
        <!-- 模板排序弹窗 -->
        <subTplManageSortDailog :dialogSortVisible="sortShow" @closeSubSortManage="handleCloseSortDailog" :tableData="tableData" :currentTreeNode="currentTreeNode" :form="form" v-if="sortShow"></subTplManageSortDailog>
        <!-- 复制模板弹窗 -->
        <subTplManageCopyDailog :copyType="copyType" :copyTreeData="copyTreeData" :dialogCopyVisible="copyShow" :multipleSelection="multipleSelection" @closeSubCopyManage="handleCloseCopyDailog" @actionUpdateTree="handleUpdateTree" :title="Fames" v-if="copyShow"></subTplManageCopyDailog>
        <!-- 右击菜单弹窗 -->
...
<script type="text/javascript">
    ...
    export default {
        ...
        methods: {
            ...
            getIsLock(param) { //获取文档是否可操作
                let data = {
                    Mr_Code: param
                }
                console.log(data)
                this.GetIsLock(data).then(res => {
                    this.isHandleDocCode = res.code
                })
            },
            // 左边树节点添加
            ...
            handleCreateNew() {
                ...
                this.DOCTOR_DELETEITEM(params).then(res => {
                    if (res.code == 2) {
            ...
            actionCloseContextMenu() {
                ...
                let menu = document.querySelector("#menu")
                menu.style.cssText = "left: -1000000px"
            ...
            get_GetInformation() {
                ...
                this.GET_TREEINFOLISTBYID(params).then((res) => {
                    if (res.code == 1) {
            ...
            get_GetSyInformation() {
                ...
                this.DOCTOR_GETTREELIST({
                    DeptID: this.deptID
                }).then((res) => {
                    this.treeData = res.values
                    this.copyTreeData = res.values
            ...
            handleCopyClicFn() {
                ...
                // 权限
                this.DOCTORGETSYSWORK(param).then(res => {
                    if (res.code == 5) {
                        this.copyShow = true
                        this.Fames = '移动模板'
                    } else if (res.code == -1) {
            ...
            handleAdd() {
                ...
                // 权限
                this.DOCTORGETSESSIONRESULT(params).then(res => {
                    if (res.code == 2) {
            ...
            handleDele() {
                ...
                if (this.multipleSelection.length == 0) {
                    ...
                    this.DOCTORGETDELETESYS(param).then(res => {
                        if (res.code == 4) {
            ...
            handleUpdateTree(ndd) {
                ...
                // this.$refs.tempTree.setCurrentKey(ndd.id)
            },
            handleClickCopy() {
                // 复制模板
                this.copyType = 1
