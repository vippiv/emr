    ...
    export default {
        ...
        data() {
            return {
                ...
                rightDepartmentChoosedRow: null,
        ...
        created() {
            this.get_information() // 查左侧数据
            this.get_SyInformation(0) // 查右侧部门数据
            this.get_SyInformation(1) // 查右侧个人数据
        },
        mounted() {
        ...
        methods: {
            ...
            get_SyInformation(department) {
                ...
                this.personData = []
                this.depatmentData = []
                this.GET_SECONDRIGHTTAB(params).then((res) => {
                    if (res.code === 1) {
                        if (department == 1) {
                            res.values.forEach((item, index) => {
                                this.personData.push({
                                    dateStamp: new Date().getTime() + index,
                                    ...item
                                })
                            })
                        } else {
            ...
            get_information() {
                ...
                this.GET_ICDBYOPERATION(params).then((res) => {
